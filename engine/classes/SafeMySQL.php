...
class SafeMySQL
{
	...
	function __construct($opt = array())
	{
		...
		if ( !$this->conn )
		{
			$this->error(mysqli_connect_errno() . " safemysql.class.php" .mysqli_connect_error());
		}
	...
	protected function prepareQuery($args)
	{
		...
		$array = preg_split('~(\?[nsiuapmk])~u',$raw,null,PREG_SPLIT_DELIM_CAPTURE);
		$anum  = count($args);
		...
		foreach ($array as $i => $part)
		{
			...
			switch ($part)
			{
				...
				case '?u':
					...
					break;
                case '?m': // MULTIARRAY
                    $part = $this->createMultiSET($value);
                    break;
                case '?k': // ARRAY KEYS
                    $part = $this->createKeysSet($value);
                    break;
				case '?p':
					$part = $value;
					break;
    ...
    // UPDATE BY ARTYOM RYBIN, 2021

    protected function createMultiSET($data) {
        if(!is_array($data)) {
            $this->error("VALUES ?m placeholder expects array, ".gettype($data)." given");
            return;
        }

        if(!is_array(reset($data))) {
            $this->error("Multidimensional array expects, in values ".gettype($data)." given");
            return;
        }
        ...
        foreach($data as $value) {
            $query .= $outerComma . '(';
            $innerComma = '';
            foreach($value as $key => $insetValue) {
                $query .= $innerComma.$this->escapeString($insetValue);
                $innerComma = ', ';
            }
            $query .= ')' ;
    ...
    protected function createKeysSet($data) {
        if(!is_array($data)) {
            $this->error("?k placeholder expects array, ".gettype($data)." given");
            return;
        }

        if (!$data) {
            $this->error("Empty array for ?k placeholder");
            return;
        }
        ...
        $query .= '(';
        foreach($data as $key => $value) {
            $query .= $comma.$this->escapeIdent($key);
            $comma = ',';
        }
        $query .= ')';
